...
vec3 clip_aabb(vec3 aabbMin, vec3 aabbMax, vec3 prevSample, vec3 avg)
{
...
void main() 
{
	...
	ivec2 imageLocation = ivec2(gl_WorkGroupID.xy);
	...
	float oneDividedBySampleCount = 0.0f;
	...
	for (int i = -1; i <= 1; ++i)
	{
		for (int j = -1; j <= 1; ++j)
		{
			...
			++oneDividedBySampleCount;
		}
	}

	oneDividedBySampleCount = 1.0f / oneDividedBySampleCount;
	...
	{
		imageStore(u_Output, imageLocation, vec4(currentColor, 1.0f));
		return;
	}
	...
	historyColor = clip_aabb(minc, maxc, historyColor, mu);

	vec3 result = alpha * currentColor + (1 - alpha) * historyColor;

	imageStore(u_Output, imageLocation, vec4(result, 1.0f));
}